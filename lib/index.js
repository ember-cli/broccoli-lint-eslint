...
const resolveInputDirectory = inputNode => {
...
class _EslintValidationFilter extends Filter {
  /**
  ...
  constructor(inputNode, options) {
    const _console = options.console || console;
    ...
    if (isString(this.internalOptions.testGenerator)) {
      ...
      if (!this.testGenerator) {
        throw new Error(
          `Could not find '${
            this.internalOptions.testGenerator
          }' test generator.`
        );
      }
    } else {
    ...
    this.targetExtension = this.testGenerator ? 'lint-test.js' : 'js';
  }
  ...
  getDestFilePath(relativePath) {
    ...
    const filterPath = super.getDestFilePath(...arguments);
...
// This is so that we remain backwards compatibile. Some people may invoke
// without the `new` keyword (which is required for ES6 classes).
const EslintValidationFilter = new Proxy(_EslintValidationFilter, {
