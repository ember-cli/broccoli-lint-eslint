...
function filterAllIgnoredFileMessages(result) {
  ...
  result.results.map(function (resultItem) {
...
EslintValidationFilter.prototype.processString = function (content, relativePath) {
  ...
  if (filteredResults.results.length) {
    ...
    if (getResultSeverity(filteredResults.results) > 0 && 'throwOnError' in this.internalOptions && this.internalOptions.throwOnError === true) {
