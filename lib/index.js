...
function EslintValidationFilter(inputNode, options) {
  ...
  // default is to not fix files
  if (typeof eslintOptions.fix === 'undefined') {
    eslintOptions.fix = false;
  }
  this.applyFixes = eslintOptions.fix;
...
EslintValidationFilter.prototype.processString = function processString(content, relativePath) {
  ...
  const output = this.cli.executeOnText(content, filePath);
  const { results } = output;
  const [ firstResult ] = results;

  if (this.applyFixes && firstResult.output) {
    content = firstResult.output;
  }
